...
* Любая функция, которая обращается к NVS, сначала вызывает
  `preferences.begin(namespace, readOnly)` и обязательно завершает работу с
  пространством через `preferences.end()` перед выходом. Это исключает ситуации
  с незакрытыми дескрипторами.
